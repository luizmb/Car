...
struct LaneHintTests {
    ...
    /// Withdrawn on the rider's correction: markings override the after-12 default often enough
    /// that the advice was sometimes wrong — and sometimes-wrong advice poisons the right kind.
    @Test("Roundabouts get no lane advice")
    func roundaboutsStaySilent() {
        let late = [
            step("At the roundabout, take the third exit onto A505",
                 path: [(52.000, -0.46), (52.002, -0.46)]),
            step("Continue", path: [(52.002, -0.46), (52.002, -0.457)])
        ]
        #expect(laneHint(late, at: 0) == nil)
    }
